...
5. **port=5353 НЕ менять на 53** — это архитектурное решение:
   - dnsmasq слушает на 5353 (избегаем конфликта с системным dnsmasq Keenetic)
   - iptables добавляет DNAT правило: порт 53 → 5353
   - Устройства используют стандартный DNS (53), который перенаправляется на dnsmasq
   - Менять только если полностью переделать архитектуру DNS
