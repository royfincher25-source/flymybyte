...
- **Global scope**: `H:/disk_e/dell/FlyMyByte/.opencode/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `H:/disk_e/dell/FlyMyByte/.opencode/notes/`)
...
2. Glob `H:/disk_e/dell/FlyMyByte/.opencode/notes/*.md` (if directory exists) — global notes
...
Global (H:/disk_e/dell/FlyMyByte/.opencode/notes/):
...
2. **No `.planning/`**: Falls back to global `H:/disk_e/dell/FlyMyByte/.opencode/notes/` — works in any directory
