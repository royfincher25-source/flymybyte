...
<template>
@H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/templates/UAT.md
</template>
...
```bash
INIT=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```
UI_PHASE_FLAG=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_phase --raw 2>/dev/null || echo "true")
...
```bash
CHECKPOINT=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
...
- Follow @H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/workflows/diagnose-issues.md
