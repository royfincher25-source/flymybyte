...
```bash
PHASE_INFO=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}")
```
...
```bash
PHASE_CHECK=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${PHASE}" --pick found)
...
```bash
PHASE_DIR=$(node "H:/disk_e/dell/FlyMyByte/.opencode/get-shit-done/bin/gsd-tools.cjs" find-phase "${PHASE}" --raw)
```
