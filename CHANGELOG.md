...
- **DNSMonitor** — увеличен интервал проверки с 60 до 300 секунд (5 минут) для значительного снижения нагрузки на CPU
- **DNS_TIMEOUT** — уменьшен с 3 до 2 секунд (оптимально для локальных DNS)
- **DNS_FAILURE_THRESHOLD** — увеличен с 3 до 5 (меньше ложных переключений)
- **Логирование DNS** — успешные проверки логируются только каждый 10-й раз для снижения I/O нагрузки
- **check_dns_server()** — оптимизировано управление сокетами для предотвращения утечек ресурсов
...
- **FILES_TO_UPDATE** — добавлены `dnsmasq_manager.py` и `iptables_manager.py` (критичные модули, без них интерфейс не запускался)

## [2.0.5] - 2026-04-07

### 🐛 Исправлено

- **OTA обновление** — последовательная загрузка файлов (max_workers=1 вместо 3) для предотвращения DNS-перегрузки на роутере
- **DNS timeout** — уменьшен с 60с до 15с для быстрого обнаружения проблем
- **Двухфазное обновление** — перезапуск сервисов (S99unblock, S56dnsmasq) только после успешной загрузки всех файлов
- **FILES_TO_UPDATE** — удалены несуществующие файлы (routes_service.py, env_parser.py, дубликат app.py)

## [2.0.4] - 2026-04-07

### 🐛 Исправлено

- **Обновление** — добавлены повторные попытки (до 3) при DNS-ошибках загрузки файлов
- **DNS retry** — экспоненциальная задержка (5с, 10с, 15с) при `NameResolutionError`
- **Устойчивость** — другие ошибки сети также повторяются (3с, 6с, 9с)

## [2.0.3] - 2026-04-07

### 🔧 Изменено

- **Диагностика** — обновлена документация по проверке портов (BusyBox `nc` не поддерживает `-w`, использовать `nc < /dev/null`)

## [2.0.2] - 2026-04-05

### 📝 Документация

- **README.md** — полностью переписан, актуальная архитектура v2.0, убраны все before/after сравнения
- Удалено 10 исторических файлов (планы, сравнения, устаревшие инструкции)
- Версия в OBSIDIAN_INSTRUCTION.md и AGENTS.md обновлена до 2.0.1
...
- **Обновление теперь не повреждает бинарные файлы** — шрифты (.woff2) и изображения скачиваются в бинарном режиме (`response.content` + `wb`), а не в текстовом
- Раньше файл `flymybyte-icons.woff2` увеличивался с 3636 до 6564 байт после обновления, иконки не работали

## [1.23.0] - 2026-04-05

### 🐛 Исправлено

- **Шрифт иконок** — добавлена явная регистрация MIME-type `font/woff2` в Flask
- **.gitattributes** — предотвращение повреждения бинарных файлов (.woff2) при git-операциях
- **Резервное копирование** — удалён дубликат `UPDATE_BACKUP_FILES`, из-за которого не сохранялись ключи Trojan, Hysteria 2, dnsmasq.conf, crontab, скрипты

## [1.22.0] - 2026-04-05

### ✨ Новое

- **Сохранение ключей при выключении** — конфиг больше не удаляется, toggle может повторно включить сервис без повторного ввода ключа
